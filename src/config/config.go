...
// exposed by the Elytra webserver.
...
// from Elytra to the Panel.
type RemoteQueryConfiguration struct {
	// The amount of time in seconds that Elytra should allow for a request to the Panel API
	...
	// Elytra instance. A single request is initially made to the Panel to get this number
...
type SystemConfiguration struct {
	// The root directory where all of the elytra data is stored at.
	RootDirectory string `default:"/var/lib/elytra" json:"-" yaml:"root_directory"`

	// Directory where logs for server installations and other elytra events are logged.
	LogDirectory string `default:"/var/log/elytra" json:"-" yaml:"log_directory"`

	// Directory where the server data is stored at.
	Data string `default:"/var/lib/elytra/volumes" json:"-" yaml:"data"`

	// Directory where server archives for transferring will be stored.
	ArchiveDirectory string `default:"/var/lib/elytra/archives" json:"-" yaml:"archive_directory"`

	// Directory where local backups will be stored on the machine.
	BackupDirectory string `default:"/var/lib/elytra/backups" json:"-" yaml:"backup_directory"`

	// TmpDirectory specifies where temporary files for Elytra installation processes
	// should be created. This supports environments running docker-in-docker.
	TmpDirectory string `default:"/tmp/elytra" json:"-" yaml:"tmp_directory"`

	// The user that should own all of the server files, and be used for containers.
	Username string `default:"pyrodactyl" yaml:"username"`

	// The timezone for this Elytra instance. This is detected by Elytra automatically if possible,
	// and falls back to UTC if not able to be detected. If you need to set this manually, that
	// can also be done.
	//
	// This timezone value is passed into all containers created by Elytra.
	...
	Passwd struct {
		...
		//
		// By default this option is disabled and Elytra will not mount any additional passwd
		...
		// Elytra. If you are running Elytra itself in a container, this path will need to be mounted
		// into the Elytra container as the exact path on the host, which should match the value
	...
	// If set to false Elytra will not attempt to write a log rotate configuration to the disk
...
type CrashDetection struct {
	...
	// Determines if Elytra should detect a server that stops with a normal exit code of
	// "0" as being crashed if the process stopped without any Elytra interaction. E.g.
...
type Backups struct {
	...
	// CompressionLevel determines how much backups created by elytra should be compressed.
	//
	...
	// Rustic configuration for backup repositories
	Rustic RusticConfiguration `yaml:"rustic"`
}
...
type RusticConfiguration struct {
	...
	Local LocalRepositoryConfig `yaml:"local"`

	// S3 repository configuration
	S3 S3RepositoryConfig `yaml:"s3"`

	// Repository version to use when initializing new repositories
	RepositoryVersion int `default:"2" yaml:"repository_version"`

	// Pack size configuration for performance tuning
	TreePackSizeMB int `default:"4" yaml:"tree_pack_size_mb"`
	DataPackSizeMB int `default:"32" yaml:"data_pack_size_mb"`
}

// LocalRepositoryConfig defines configuration for local rustic repositories
type LocalRepositoryConfig struct {
	// Whether local backups are enabled
	Enabled bool `default:"true" yaml:"enabled"`

	// Base directory for local repositories
	RepositoryPath string `default:"/var/lib/elytra/rustic-repos" yaml:"repository_path"`

	// Whether to use hot/cold storage setup for local repos
	UseColdStorage bool `default:"false" yaml:"use_cold_storage"`

	// Path for hot storage if using cold storage setup
	HotRepositoryPath string `yaml:"hot_repository_path"`
}

// S3RepositoryConfig defines configuration for S3 rustic repositories
type S3RepositoryConfig struct {
	// Whether S3 backups are enabled
	...
	// Hot bucket name if using cold storage setup
	HotBucket string `yaml:"hot_bucket"`

	// Storage class for cold storage
	ColdStorageClass string `default:"GLACIER" yaml:"cold_storage_class"`

	// Force path-style addressing (required for some S3-compatible services)
	// Set to true for MinIO, some custom S3 implementations
	ForcePathStyle bool `default:"false" yaml:"force_path_style"`

	// Disable SSL/TLS (only for development/testing)
	DisableSSL bool `default:"false" yaml:"disable_ssl"`
...
type Configuration struct {
	...
	// Determines if elytra should be running in debug mode. This value is ignored
	// if the debug flag is passed through the command line arguments.
	Debug bool

	AppName string `default:"Elytra" json:"app_name" yaml:"app_name"`
	...
	// is only required by users running Elytra without SSL certificates and using internal IP
...
// ConfigurePasswd generates required passwd files for use with containers started by Elytra.
...
// EnableLogRotation writes a logrotate file for elytra to the system logrotate
...
func EnableLogRotation() error {
	if !_config.System.EnableLogRotate {
		log.Info("skipping log rotate configuration, disabled in elytra config file")
		return nil
	}
	...
	if _, err := os.Stat("/etc/logrotate.d/elytra"); err == nil || !os.IsNotExist(err) {
		return err
	}
	...
	// but there is not a file for elytra already. In that case, let us write a new file to
	// it so files can be rotated easily.
	f, err := os.Create("/etc/logrotate.d/elytra")
	if err != nil {
		return err
	}
	defer f.Close()

	t, err := template.New("logrotate").Parse(`{{.LogDirectory}}/elytra.log {
    size 10M
    compress
    delaycompress
    dateext
    ...
    notifempty
    postrotate
        /usr/bin/systemctl kill -s HUP elytra.service >/dev/null 2>&1 || true
    endscript
}`)
	if err != nil {
		return err
...
func ConfigureTimezone() error {
	...
	if _config.System.Timezone == "" {
		...
		if err != nil {
			...
			// fall through to UTC to get Elytra booted at least.
